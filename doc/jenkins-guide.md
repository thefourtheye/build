...
First, get statistics on running processes for the machine: `ps aux | grep node | grep -v -egrep -ejava`.

If there are a lot of "hanging" / "abandoned" processes, it's best to
remove them by running a command like: `ps -ef | grep node | grep -v -egrep -ejava | awk '{print $2}' | xargs kill`.
